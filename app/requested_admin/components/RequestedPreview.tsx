...
export const RequestedPreview = () => {
  const [previewProps] = useAtom(previewConfigAtom);
  ...
  useEffect(() => {
    ...
    const col = collection(db, previewProps.questionType);

    const unsubscribe = onSnapshot(col, (snapshot) => {
      const foundDoc = snapshot.docs.find((doc) => doc.id === previewProps.id);
  ...
  if (!previewProps || !requestedQuizList) return;
